                                ...
                                try:
                                    ...
                                    for company in root.findall(".//list"):
                                        ...
                                        stock_code = (
                                            ...
                                            if stock_code_elem is not None
                                            else ""
                                        )
                                        ...
                                        if name and corp_name in name:
                                            ...
                                            if name != corp_name:
                                                ...
                                                if not name.startswith(corp_name):
                                                    score += 10
                                            # 상장사(stock_code 있음)를 비상장사보다 우선 (동일 이름인 경우)
                                            if (
                                                not stock_code
                                                or stock_code.strip() == ""
                                            ):
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                try:
                    ...
                    return (
                        "다운로드한 파일이 유효한 ZIP 파일이 아닙니다 (내용 확인 불가)."
                    )
            except Exception as e:
                return f"ZIP 파일 처리 중 오류 발생: {str(e)}"
...
def detect_namespaces(
    xbrl_content: str, base_namespaces: Dict[str, str]
) -> Dict[str, str]:
    """
...
def parse_xbrl_financial_data(
    xbrl_content: str, items_and_tags: Dict[str, List[str]]
) -> Dict[str, str]:
    """
    ...
    try:
        ...
        # 네임스페이스 추출 및 업데이트
        namespaces, detected_namespaces = detect_namespaces(
            xbrl_content, base_namespaces
        )
                    ...
                    for elem in elements:
                        ...
                        if context_ref and pattern_regex.match(context_ref):
                            ...
                            if value_text and unit_ref:
                                try:
                                    formatted_value = format_numeric_value(
                                        value_text, decimals
                                    )
                                    extracted_data[item_name] = (
                                        f"{formatted_value} ({report_type})"
                                    )
                                    item_found = True
                                    break
...
def extract_business_section(document_text: str, section_type: str) -> str:
    ...
    matching_titles = [
        title for title in title_tags if section_keyword.lower() in title.lower()
    ]
    ...
    if not matches:
        ...
        pattern = section_patterns[section_type].replace(
            "</SECTION", "</SECTION(?:-\\d+)?"
        )
        matches = re.search(pattern, document_text, re.DOTALL | re.IGNORECASE)
    ...
    if not matches and matching_titles:
        for title in matching_titles:
            ...
            matches = re.search(
                direct_pattern, document_text, re.DOTALL | re.IGNORECASE
            )
            if matches:
                break
                    ...
                    with zip_file.open(target_file) as doc_file:
                        ...
                        if file_ext in ["txt", "html", "htm", "xml", "xbrl"]:
                            ...
                            if text_content:
                                ...
                                return (
                                    "파일을 텍스트로 변환할 수 없습니다 (인코딩 문제).",
                                    file_content,
                                )
                        # PDF 또는 기타 바이너리 파일
                        else:
                            return (
                                f"파일이 텍스트 형식이 아닙니다 (형식: {file_ext}).",
                                file_content,
                            )
...
async def search_disclosure(
    ...
    try:
        ...
        if requested_items:
            items_to_extract = {
                item: tags
                for item, tags in all_items_and_tags.items()
                if item in requested_items
            }
            if not items_to_extract:
                unsupported_items = [
                    item for item in requested_items if item not in all_items_and_tags
                ]
        ...
        for disclosure in disclosures[:disclosure_count]:
            ...
            try:
                ...
                if not xbrl_text.startswith(
                    ("DART API 오류:", "API 요청 실패:", "ZIP 파일", "<인코딩 오류:")
                ):
                    try:
                        financial_data = parse_xbrl_financial_data(
                            xbrl_text, items_to_extract
                        )
                    except Exception as e:
                        parse_error = e
                    ...
                    financial_data = {
                        key: "데이터 없음(API 013)" for key in items_to_extract
                    }
                else:
                    error_summary = xbrl_text.split("\n")[0][:100]
                    financial_data = {
                        key: f"오류({error_summary})" for key in items_to_extract
                    }
...
async def search_business_information(
    ...
    try:
        ...
        # 섹션 추출
        try:
            section_text = await extract_business_section_from_dart(
                rcept_no, information_type
            )

            # 추출 결과 확인
            if section_text.startswith(
                f"공시서류 다운로드 실패"
            ) or section_text.startswith(f"'{information_type}' 섹션을 찾을 수 없습니다"):
                ...
                result += (
                    "1. 해당 공시에 요청하신 정보가 포함되어 있지 않을 수 있습니다.\n"
                )
        ...
        if statement_type is None:
            ...
            for sj_div, data in statement_data.items():
                ...
                for item in data:
                    ...
                    if reprt_code in ["11012", "11013", "11014"] and sj_div in [
                        "IS",
                        "CIS",
                    ]:
                        thstrm_amount = (
                            item.get("thstrm_add_amount", thstrm_amount)
                            or thstrm_amount
                        )
            ...
            if not data:
                ...
                for item in data:
                    ...
                    # 3분기나 1분기, 반기 보고서이면서 손익계산서인 경우 당기 누적금액도 고려
                    if reprt_code in ["11012", "11013", "11014"] and statement_type in [
                        "IS",
                        "CIS",
                    ]:
                        thstrm_amount = (
